...
export class BedrockKBSearcher {
  ...
  private async _init(): Promise<void> {
    try {
      const { BedrockAgentRuntimeClient } = await import(
      ...
      console.error(
        `knowledge-search: Failed to initialize Bedrock KB client: ${err.message}`
      );
      this.configs = [];
    }
  }
  ...
  async search(
    ...
    const { RetrieveCommand } = await import(
      "@aws-sdk/client-bedrock-agent-runtime"
    );
    ...
    const searches = this.configs.map(async (config) => {
      ...
      try {
        ...
        console.error(
          `knowledge-search: KB ${config.id} search failed: ${err.message}`
        );
        return [];
      }
    });
